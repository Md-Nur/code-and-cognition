...
export default async function ClientPortalPage(props: {
    ...
    // Check access rights
    const isMagicLink = project.viewToken === viewToken;
    const isFounder = session?.user && (session.user.role === "FOUNDER" || session.user.role === "CO_FOUNDER");
    const isClientAccount = session?.user && project.clientUserId === session.user.id;

    if (!isMagicLink && !isClientAccount && !isFounder) {
        if (!session?.user) {
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 font-sans">
            ...
            <header className="fixed w-full top-0 border-b border-gray-200 dark:border-gray-800 bg-white/80 dark:bg-gray-950/80 backdrop-blur-md z-50">
                <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
                    ...
                    <div
                        className={`flex items-center gap-2 px-4 py-2 rounded-full font-medium text-sm ${currentHealth.bg} ${currentHealth.text}`}
                    >
                        {currentHealth.icon}
                                ...
                                <div className="space-y-8 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-linear-to-b before:from-transparent before:via-gray-300 dark:before:via-gray-700 before:to-transparent">
                                    {project.milestones.map((milestone: any) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <div
                                                    className={`flex items-center justify-center w-10 h-10 rounded-full border-4 border-white dark:border-gray-900 ${isCompleted ? "bg-emerald-500" : isInProgress ? "bg-blue-500" : "bg-gray-300 dark:bg-gray-700"} shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 shadow-sm z-10`}
                                                >
                                                    {isCompleted ? (
                                                ...
                                                <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-5 rounded-2xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-800 md:group-odd:text-right hover:border-blue-500/30 transition-colors">
                                                    <div className="flex flex-col md:group-odd:items-end">
                                                        <span
                                                            className={`text-[10px] uppercase font-bold tracking-widest ${isCompleted ? "text-emerald-500" : isInProgress ? "text-blue-500" : "text-gray-400"}`}
                                                        >
                    ...
                    <div className="sticky top-28 space-y-6">
                        ...
                        <div className="bg-blue-50 dark:bg-blue-900/10 rounded-3xl p-6 border border-blue-100 dark:border-blue-800/30 text-center">
                            <p className="text-sm text-blue-800 dark:text-blue-300">
                                ...
                                <a
                                    href="#"
                                    className="font-bold border-b border-blue-800 dark:border-blue-300 mt-2 inline-block hover:text-blue-600 transition-colors"
                                >
