...
export async function POST(req: Request, { params }: Params) {
    ...
    const isMember = project.members.some(m => m.userId === session.user.id);
    if (session.user.role !== Role.FOUNDER && !isMember) {
        return NextResponse.json({ error: "Forbidden — only Founders/Contractors can create change requests" }, { status: 403 });
    }

    const body = await req.json();
    const { title, description, estimatedTimeImpact, estimatedBudgetImpact } = body;
...
export async function PATCH(req: Request, { params }: Params) {
    ...
    if (!crId || !["APPROVED", "REJECTED"].includes(status)) {
        return NextResponse.json({ error: "crId and valid status (APPROVED|REJECTED) required" }, { status: 400 });
    }

    // Only FOUNDER can approve/reject (acting as the client-side representative)
    if (session.user.role !== Role.FOUNDER) {
        return NextResponse.json({ error: "Forbidden — only Founders/Clients can approve or reject change requests" }, { status: 403 });
    }
