...
export default async function SubCategoryDetailPage({ params }: PageProps) {
  ...
  return (
    <main className="min-h-screen bg-agency-black selection:bg-agency-accent selection:text-white">
      ...
      <section className="pt-32 pb-20 border-b border-white/5 bg-gradient-to-b from-agency-accent/10 to-transparent">
        <div className="section-container">
          <nav className="flex items-center gap-2 text-sm text-gray-500 mb-8 overflow-x-auto whitespace-nowrap pb-2 scrollbar-none">
            <Link
              href="/services"
              className="hover:text-agency-accent transition-colors"
            >
              Services
            </Link>
            <span>/</span>
          ...
          <div className="flex flex-col gap-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-1 gap-12">
              <div>
                ...
                <h1 className="text-4xl md:text-7xl font-bold tracking-tight mb-12">
                  {subCategory.title}
                </h1>
          ...
          <div className="section-container">
            <div className="flex items-center justify-between mb-12">
              <div>
                ...
                <p className="text-gray-500 mt-2">
                  See how we've helped other clients with {subCategory.title}.
                </p>
              </div>
              <Link
                href="/portfolio"
                className="text-agency-accent font-bold hover:underline hidden sm:block"
              >
                View All Projects
              </Link>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {subCategory.portfolioItems.map((item) => (
                <div
                  key={item.id}
                  className="group relative rounded-2xl overflow-hidden aspect-square glass-panel border border-white/5"
                >
                  {item.imageUrl ? (
                    <img
                      src={item.imageUrl}
      ...
      <BookingForm defaultServiceId={subCategory.serviceId} />
      <Footer />
    </main>
  );
}
