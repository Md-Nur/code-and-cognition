...
function StatusBadge({ status }: { status: CRStatus }) {
    ...
    return (
        <span className={`text-[10px] px-2 py-0.5 rounded border font-semibold uppercase tracking-wider ${styles[status]}`}>
            {status}
        </span>
    );
}

const EMPTY_FORM = { title: "", description: "", estimatedTimeImpact: 0, estimatedBudgetImpact: 0 };

export default function ChangeRequestsPanel({ projectId, initialCRs, userRole }: Props) {
    ...
    async function refreshCRs() {
        ...
        if (res.ok) setCRs(await res.json());
    }
        ...
        <div className="glass-panel p-6 rounded-xl">
            ...
            <div className="flex justify-between items-center mb-6">
                <h3 className="text-lg font-bold flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-agency-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            {/* Table */}
            {crs.length === 0 ? (
                <p className="text-gray-500 text-sm italic text-center py-6">No change requests yet.</p>
            ) : (
                            ...
                            <div className="flex justify-between items-start gap-4">
                                <div className="flex-1 min-w-0">
                                    <div className="flex flex-wrap items-center gap-2 mb-1">
                                        <h4 className="font-semibold text-sm truncate">{cr.title}</h4>
                                        <StatusBadge status={cr.status} />
                                    </div>
                                    <p className="text-xs text-gray-400 line-clamp-2 mb-2">{cr.description}</p>
                                    <div className="flex flex-wrap gap-4 text-[10px] text-gray-500">
                                        <span>By <span className="text-gray-300">{cr.requestedBy.name}</span></span>
                                        {cr.estimatedTimeImpact > 0 && (
                                            <span>⏱ +{cr.estimatedTimeImpact}h</span>
                                        )}
                                        {cr.estimatedBudgetImpact > 0 && (
                                            <span>💰 +{cr.estimatedBudgetImpact.toLocaleString()}</span>
                                        )}
                                        <span>{new Date(cr.createdAt).toLocaleDateString()}</span>
                                        {cr.clientApprovalAt && (
                                            <span>Decided: {new Date(cr.clientApprovalAt).toLocaleDateString()}</span>
                                        )}
                                    </div>
                                </div>
            ...
            {showModal && (
                <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="glass-panel w-full max-w-lg p-6 rounded-xl animate-fade-in-up">
                        ...
                        <p className="text-xs text-gray-500 mb-5">Describe the scope change. A Founder will review and approve or reject it.</p>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="input-label">Estimated Time Impact (hours)</label>
                                    <input
                                        type="number"
                                        min="0"
                                        step="0.5"
                                        ...
                                        onChange={e => setForm({ ...form, estimatedTimeImpact: parseFloat(e.target.value) || 0 })}
                                    />
                                </div>
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={e => setForm({ ...form, estimatedBudgetImpact: parseFloat(e.target.value) || 0 })}
                                    />
                                </div>
                            </div>
